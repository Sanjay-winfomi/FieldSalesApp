...
describe('syncManager', () => {
  ...
  test('flushQueue sends queued actions and removes them on success', async () => {
    ...
    expect(api.request).toHaveBeenCalledWith(
      expect.objectContaining({ method: 'post', url: '/attendance/login' })
    );
    expect(await getPendingCount()).toBe(0);
  });
  ...
  test('reconciles a 409 conflict via the conflict handler instead of silently dropping it', async () => {
    await enqueueAction('post', '/attendance/logout', { attendance_id: 5, lat: 1, lng: 2 });
    api.request.mockRejectedValueOnce({
      response: { status: 409, data: { error: 'Already logged out today', attendance: { id: 5, logout_time: 'now' } } },
    });
    ...
    expect(conflictHandler).toHaveBeenCalledWith(
      expect.objectContaining({ serverError: 'Already logged out today', attendance: { id: 5, logout_time: 'now' } })
    );
